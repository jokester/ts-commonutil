...
import { toPromise1, toPromise2 } from '../type/promisify';
...
namespace FSImpl {
  ...
  export const readDir = toPromise1<string, string[]>(fs.readdir);
  ...
  export async function readDirDetail(dirName: string): Promise<DirItem[]> {
    ...
    const children: Promise<DirItem>[] = childNames.map(async (name: string) => {
...
export interface FSType {
  ...
  readText(filename: string, options: { encoding: string; flag?: string }): Promise<string>;
